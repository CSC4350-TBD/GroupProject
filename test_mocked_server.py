    ...
    def test_get_imdb_id(self):
        with patch("imdb.requests.get") as mock_requests_get:
            ...
            mock_response.json.side_effect = [
                {"results": [{"id": "mockedmovieid", "image": {"mockedmovieimage"}}]}
            ]
            mock_requests_get.return_value = mock_response
            self.assertEqual(
                get_imdb_id("Movie Name"), ("mockedmovieid", {"mockedmovieimage"})
            )
...
class GetMovieRecs(unittest.TestCase):
    def test_get_movie_recs(self):
        with patch("imdb.requests.get") as mock_requests_get:
            ...
            mock_requests_get.return_value = mock_response
            self.assertEqual(
                get_movie_recs("movie Name"),
                ["mockedmovieid1", "mockedmovieid2", "mockedmovieid3"],
            )
